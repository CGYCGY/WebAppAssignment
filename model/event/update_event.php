...
if(!empty($_POST)) {
    ...
    $query = 'UPDATE `event` SET `event_name` = ?, `event_description` = ?, `event_location` = ?, `event_org_name` = ?, `event_contact` = ?, `event_dt_start` = ?, `event_dt_end`=? WHERE `event_id` = ?';
    $param = array($eName, $eDesc, $eLoc, $eOrgName, $OrgCon, $eDTStart, $eDTEnd, $eID);
